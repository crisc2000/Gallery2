...
if ($gallery->isEmbedded()) {
	...
	if ($unsanitizedView == 'core.DownloadItem' && !empty($itemId)) {
		 ...
		 */
		if (isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) || (function_exists('getallheaders')
				&& ($headers = getallheaders()) && (isset($headers['If-Modified-Since'])
...
function GalleryMain($embedded=false) {
	...
	if ($ret) {
		...
		print '<h1> Debug Redirect </h1>Not automatically redirecting to the next page because we are in debug mode<br/>';
...
function _GalleryMain_doRedirect($redirectUrl, $template=null, $controller=null, $ignoreErrors=false) {
	...
	if ($gallery->getDebug() == false || $gallery->getDebug() == 'logged') {
		...
		if (in_array($controller, array('core.Logout', 'core.UserLogin', 'publishxp.Login'))) {
			...
			if (!empty($webserver) && preg_match('|^Microsoft-IIS/(\d)\.\d$|', trim($webserver), $matches) && $matches[1] < 6) {
				/* if it is an IIS version with this bug, check if GALLERYSID is already in the  URL, else append it */
				$session =& $gallery->getSession();
	...
	return array(null, array('isDone' => true, 'redirectUrl' => $redirectUrl, 'template' => $template));
}
